...
export class WarenkorbComponent implements OnInit {
  ...
  url = 'http://168.119.224.182:3000/order';
  ...
  public async processPayment() {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: 'http://168.119.224.182',
      cancel_url: 'http://168.119.224.182',
    });
  ...
  public BestellungAbschliesen(){
    ...
    const adresse = this.contactComponent.AdresseAnWarenkorb();
    const Adresse = {
      Vorname:         adresse.lastName,
      Nachname:        adresse.firstName,
      Telefonnummer:   adresse.phone,
      EMailAdresse:    adresse.email
    }

let KundeAJ = true;
for (let key in adresse) {
  adresse.message = 'a';
  if (adresse[key].trim() === '') {
    KundeAJ = false;
    console.log(`Das Feld ${key} darf nicht leer sein.`);
    this.Fehlermeldung = `Das Feld ${key} darf nicht leer sein.`;
    // Hier können Sie weitere Aktionen durchführen, z.B. den Benutzer informieren
  }
}
if (KundeAJ === true){
console.log(adresse);
      ...
      for (let cartItem of this.cart) {
        lineItems.push({
          Eventnummer: cartItem.product.Eventnummer,
